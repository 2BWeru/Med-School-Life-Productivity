...
import { createQuizResult } from './actions';
...
export function ResultForm({ subjects }: { subjects: Subject[] }) {
  ...
  async function handleAction(formData: FormData) {
    await createQuizResult(formData);
  ...
  return (
    <form ref={formRef} action={handleAction} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div className="flex flex-col gap-1.5 lg:col-span-2">
        ...
        <Input id="title" name="title" placeholder="e.g. Pharmacology quiz 3" required />
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Input id="score" name="score" type="number" placeholder="8" />
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="maxScore">Out of</Label>
        <Input id="maxScore" name="maxScore" type="number" placeholder="10" />
      </div>
